...
export default async function blockNotifications(base) {
  const miloOrigin = (typeof window !== 'undefined' && window.__CW_ASSET_ROOT)
    ? String(window.__CW_ASSET_ROOT).replace(/\/$/, '')
    : window.location.origin;
  const { blocknotifications } = await getServiceConfig(miloOrigin);
  if (blocknotifications?.enabled === 'false') return;
  ...
  if (!blocksKeyToLowercase?.data || !Array.isArray(blocksKeyToLowercase.data)) return;
