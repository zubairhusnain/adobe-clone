...
class Footer {
  ...
  init = () => logErrorFor(async () => {
    // Offline mirror: footer HTML is pre-rendered in PHP — skip fetch/append entirely.
    if (this.block.querySelector('.feds-footer-wrapper') || this.block.dataset.cwFooterStatic === '1') {
      this.block.dataset.blockStatus = 'loaded';
      return;
    }
  ...
  decorateContent = () => logErrorFor(async () => {
    // Offline mirror: keep pre-rendered footer (trimmed in PHP); do not fetch/append again.
    if (this.block.querySelector('.feds-footer-wrapper') || this.block.dataset.cwFooterStatic === '1') {
      this.block.dataset.blockStatus = 'loaded';
      return;
    }
    ...
    if (!this.block.querySelector('.feds-footer-wrapper')) {
