...
function cwInstallBase() {
  const fromWin = (window.__CW_ASSET_ROOT || '').replace(/\/$/, '');
  if (fromWin) return fromWin;
  const i = window.location.pathname.indexOf('/creativecloud');
  if (i > 0) return window.location.pathname.slice(0, i);
  return '/clon-adobe-website';
}

const CW_BASE = cwInstallBase();
...
const miloLibs = setLibs(`${CW_BASE}/libs`);
...
const CONFIG = {
  contentRoot: `${CW_BASE}/cc-shared`,
  codeRoot: `${CW_BASE}/creativecloud`,
...
export const scriptInit = async () => {
  ...
  (function loadStyles() {
    ...
    if (getMetadata('theme') === 'doodlebug') paths.push(`${CW_BASE}/creativecloud/styles/doodlebug.css`);
    paths.forEach((path) => {
