...
function getSiteRoot() {
  if (typeof window === 'undefined' || !window.__CW_ASSET_ROOT) return '';
  return String(window.__CW_ASSET_ROOT).replace(/\/$/, '');
}

function getStylesPath() {
  const root = getSiteRoot();
  return root ? `${root}/upp/styles/styles.css` : '/upp/styles/styles.css';
}
...
function miloConfigExtras(libs) {
  const root = getSiteRoot();
  return {
    ...(root ? { origin: root } : {}),
    miloLibs: libs,
...
function loadStyles() {
  const href = getStylesPath();
  if (document.querySelector(`link[href="${href}"], link[href="upp/styles/styles.css"]`)) {
    return;
  }
